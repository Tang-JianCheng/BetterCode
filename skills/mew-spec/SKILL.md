---
name: mew-spec
description: 启动功能、模块或系统性优化时先创建四份规格文档
tools:
  - read_file
  - write_file
  - edit_file
...
把想法变成可运行的代码，中间经过四份递进文档。每份在前一份基础上细化，每份都需要用户审批后才能进入下一阶段。

```
spec.md（做什么）→ plan.md（怎么做）→ task.md（按什么顺序做）→ checklist.md（做对了没）
```

**核心铁律：四份文档全部生成并获得用户批准之前，禁止编写任何实现代码。无论项目看起来多简单，一律走完流程。**

## 快速参考

| 阶段 | 产出 | 回答的问题 | 审批后才进入下一阶段 |
|------|------|-----------|---------------------|
| 一 | spec.md | 做什么 | 是 |
| 二 | plan.md | 怎么做 | 是 |
| 三 | task.md | 按什么顺序做 | 是 |
| 四 | checklist.md | 做对了没 | 是 |
| 五 | 代码 | — | 按 task.md 执行 |
| 六 | 验收 | — | 按 checklist.md 检查 |

## When to Use

- 开始任何功能、模块或章节开发前
- 用户描述了一个项目/功能想法但需求不明确
- 需要明确范围边界（哪些做、哪些不做）
- 多步骤任务，需要有序执行
- 任务看起来"太简单不需要 spec"——越是这种，越要走流程

## When NOT to Use

- 纯粹的信息查询（"这个文件是干什么的？"）
- 已经完成开发、只需要 code review 或验证的场景
- 单行修复或明显的拼写错误（不需要需求澄清的）
...
| 借口 | 现实 |
|------|------|
| 「这个太简单了，不需要写 spec」 | 越是简单的项目，未被审视的假设越多。基线测试中，"简单 CLI" 任务未经确认就定义了「段落」的含义，用户可能完全不认同。 |
| 「我直接写代码就行」 | 铁律：四份文档全过了才能动代码。违反就是跳过流程。 |
| 「spec 太明显了，直接跳到 plan」 | 「明显」意味着没被验证过。写出来让用户确认。 |
| 「checklist 等做完了再补」 | checklist 决定了你要验证什么，必须在编码前设计。事后补 = 测试已经通过的代码。 |
| 「测试过了就说明没问题」 | 测试验证代码，checklist 验证需求。两者不互相替代。 |
| 「应该没问题了」 | 跑一下。先有证据再下结论。 |
| 「我知道用户想要什么」 | 问一下。一次一个问题。基线测试中，auth 系统在没有确认需求的情况下就选定了 JWT + bcrypt。 |
| 「用户说'快点'，走流程太慢」 | 跳过流程导致返工花费的时间远超走流程的时间。 |

## 危险信号 — 停下来，回到流程

出现以下任何想法时，你在为跳过流程找理由：
...
**以上所有都意味着：回到阶段一，从 spec.md 开始。**

## 核心原则

- **铁律：** 四份文档全部审批通过后才能写代码——没有例外
- **一次一个问题**——每次聚焦一个问题
- **优先用选择题**——比开放题更容易回答
- **逐段审批**——逐步呈现，每段确认后再继续
...
- **每个小节写完整**——所有内容就绪后再提交审批
- **先有证据再下结论**——先跑验证，再报告结果
- **聚焦行为描述**——spec 和 checklist 描述系统做什么，不描述怎么做
