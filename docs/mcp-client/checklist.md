...
## 增量验收：项目根 `.mcp.json` 配置桥接

- [x] **C65：通用 JSON 格式完成转换**
  `.mcp.json` 的 `mcpServers` 同时包含 stdio 与 HTTP Server 时，两者都转换为现有配置模型；省略类型时可由 `command` 或 `url` 推断。（验证：运行配置桥接测试；覆盖 AC17）

- [x] **C66：环境变量和秘密脱敏保持一致**
  兼容配置的 env 与 headers 支持 `${VAR}`，缺失变量只禁用对应 Server，诊断不包含同字段明文或展开后的秘密。（验证：运行兼容配置环境变量测试并扫描输出；覆盖 AC17、AC20）

- [x] **C67：三层覆盖顺序固定**
  同名 Server 按用户 YAML、项目兼容 JSON、项目原生 YAML 依次覆盖，原生项目 YAML 最终胜出且不发生字段深合并。（验证：运行三层覆盖测试；覆盖 AC19）

- [x] **C68：兼容配置故障被隔离**
  非法 JSON、非法传输、缺失字段和无效兄弟 Server 只产生对应项目级诊断，用户层、项目原生层和其他有效 Server 保持可用。（验证：运行兼容配置故障矩阵；覆盖 AC20）

- [x] **C69：项目路径逃逸被拒绝**
  `.mcp.json` 是指向项目外的符号链接时不读取外部文件，诊断不包含外部配置正文，低优先级有效配置继续工作。（验证：运行兼容配置符号链接测试；覆盖 AC20）

- [x] **C70：真实工具复用统一桥接链**
  真实本地 Streamable HTTP Server 从 `.mcp.json` 启动后，其工具进入统一 ToolRegistry，并完成调用、会话复用、断线错误和关闭。（验证：运行 HTTP MCP 集成测试；覆盖 AC18）

- [x] **C71：原有路径完整回归**
  未配置 `.mcp.json` 时原生两层 YAML、内置工具、Agent Loop 和 Provider 行为不变，TypeScript 类型检查与全量测试通过。（验证：运行 `pnpm check`；覆盖 N11）
