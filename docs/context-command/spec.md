...
原网格按 `context_window / 5_000` 计算格数并用占比取整占用格，1M 窗口下 9.1k 用量只占 0.9%，`Math.round(0.009 × 格数) = 0`，导致整个网格全是空格子（`⛶`），占用完全无法直观呈现；且单行格数随列宽可达 52 个，视觉上只剩框框。

### 变更内容

- F23：每个格子代表 **5k Token**，格数 = `ceil(contextWindow / 5_000)`（1M → 200 格，128k → 26 格）；占用格数 = `round(usedTokens / 5_000)`，只要有用量就至少渲染 1 个占用格。
- F24：**每行最多 20 个格子**，超过自动换行成多行网格（1M → 10 行）；占用格按图例分类着色，跨行连续。
- F25：**分类汇总区并排在网格右侧**：同一行 = 格子前缀 + 模型名 / 总占用 / `*Estimated usage by category*` / 六个分类行（单 `⛁` / `⛶` 图标 + 文字，颜色与分类一致）。列宽不足 76 时回退为上下布局（网格在上、分类在下）。

### 验收补充

- AC14：1M 窗口渲染 200 格 × 每行 20 格 = 10 行；9.1k 用量渲染 2 个 `⛁` 占用格，占用与空闲可区分。
- AC15：分类信息显示在格子右侧并排，分类行只显示单个 `⛁` / `⛶` 图标并按分类着色；`xx tokens` 弱化保持。
- AC16：窄屏（列宽 < 76）回退为网格在上、分类在下的上下布局，不越界。
