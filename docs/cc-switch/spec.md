...
cc-switch 桌面版（farion1231/cc-switch）切换 Claude Code 供应商时改写 `~/.claude/settings.json` 的 `env` 块：
...
- F1：BetterCode 启动时读取 cc-switch 桌面版管理的 Claude Code 供应商配置，转换为 BetterCode 的 Provider，并在启用后自动成为默认供应商。
- F2：来源固定为 `~/.claude/settings.json` 的 `env` 块，不读取 Codex 等其他 cc-switch 来源。
...
- F7：只读适配：不写回 cc-switch 文件，不读取 cc-switch 的 SQLite 数据库，不影响 Claude Code 自身配置。
...
- N4：不新增解析依赖，`settings.json` 使用标准 JSON 解析；解析失败按诊断降级。
...
- AC1：配置 `cc_switch.enabled: true` 且 `~/.claude/settings.json` 存在时，启动使用导入供应商，`/status` 显示导入的 base_url 与模型。
...
- AC3：`--provider cc-switch.claude` 能显式选择导入供应商；cc-switch 关闭或不可用时回退到 config.yaml 原逻辑。
